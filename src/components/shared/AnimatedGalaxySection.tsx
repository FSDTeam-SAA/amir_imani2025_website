...
export default function MixedGalaxySymbols() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      const distance =
        Math.pow(Math.random(), 2) * (Math.min(width, height) * 0.42) + 15;
      ...
      const symbolData = isSymbol
        ? loadedSymbolImages[
            Math.floor(Math.random() * loadedSymbolImages.length)
          ]
        : null;
    ...
    <section className="bg-black text-white min-h-screen grid grid-cols-1 lg:grid-cols-12 items-center font-sans relative overflow-hidden">
      ...
      <div className="lg:col-span-5 px-8 md:px-16 gap-6 lg:px-24 py-16 flex flex-col justify-center relative z-10">
        ...
        <p className="text-stone-300 text-sm md:text-base leading-relaxed max-w-md mb-9 font-normal antialiased">
          Ask a question. Choose three symbols. Discover the patterns and
          tensions hidden beneath the surface.
        </p>
